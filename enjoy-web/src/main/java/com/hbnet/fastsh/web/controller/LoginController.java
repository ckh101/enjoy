...
import com.hbnet.fastsh.web.service.base.ProfileConfig;
import com.hbnet.fastsh.web.service.impl.UserService;
import com.hbnet.fastsh.web.service.impl.WxUserService;
...
public class LoginController {
    ...
    public String login(){
        ...
        if(ProfileConfig.DEV_PROFILE.equalsIgnoreCase(profile)){
            UsernamePasswordToken token = new UsernamePasswordToken("admin", DigestUtils.md5Hex("15889963035"));
            Subject currentSubject = SecurityUtils.getSubject();
    ...
    public ApiResponse sendPhoneCode(String phone) throws Exception {
        ApiResponse result = new ApiResponse();
        String phoneStr = (String)mcc.get("smartad_"+phone);
        if(!Tools.isBlank(phoneStr)) {
            ...
            if(flag) {
                ...
                mcc.set("smartad_"+phone, phone,Tools.addSeconds(new Date(), 55));
                result.setStatus(1);
            }else {
                result.setMsg("发送失败");
            }
